...
# ============================================================================
# Operation/filter predicates (SINGLE SOURCE OF TRUTH -- see SKILL.md S2)
# Callers: pass $hdr (header array) and $row (data array). Returns $true/$false.
# ============================================================================
function Test-PpvRow {
    <# PPV dual-match: op startswith PPV OR (blank op AND met = PPV-M SAMPLE SIZE) #>
    param([string[]]$Header, [string[]]$Row)
    $opIdx  = [array]::IndexOf($Header, 'OperationName')
    $metIdx = [array]::IndexOf($Header, 'MetricName')
    $op  = if ($opIdx -ge 0 -and $opIdx -lt $Row.Count) { $Row[$opIdx] } else { '' }
    $met = if ($metIdx -ge 0 -and $metIdx -lt $Row.Count) { $Row[$metIdx] } else { '' }
    return ($op -like 'PPV*') -or ([string]::IsNullOrWhiteSpace($op) -and $met -eq 'PPV-M SAMPLE SIZE')
}

function Test-ClassRow {
    <# Class dual-match: op startswith TEST OR (blank op AND met in MPS/EQA/CS Monitor) #>
    param([string[]]$Header, [string[]]$Row)
    $opIdx  = [array]::IndexOf($Header, 'OperationName')
    $metIdx = [array]::IndexOf($Header, 'MetricName')
    $op  = if ($opIdx -ge 0 -and $opIdx -lt $Row.Count) { $Row[$opIdx] } else { '' }
    $met = if ($metIdx -ge 0 -and $metIdx -lt $Row.Count) { $Row[$metIdx] } else { '' }
    return ($op -like 'TEST*') -or ([string]::IsNullOrWhiteSpace($op) -and $met -in @('MPS Monitor','EQA Monitor','CS Monitor'))
}

function Test-ClassPpvRow {
    <# Class+PPV union: row matches Class OR PPV #>
    param([string[]]$Header, [string[]]$Row)
    return (Test-ClassRow $Header $Row) -or (Test-PpvRow $Header $Row)
}

function Tci-ApplyFilter {
    <#
    .SYNOPSIS Apply a named filter to parsed rows. Returns filtered list (header + matching rows).
    .PARAMETER Filter  One of: PPV, Class, ClassPPV, BI, Yield, or a raw op prefix.
    #>
    param(
        [Parameter(Mandatory)][string[]]$Header,
        [Parameter(Mandatory)][System.Collections.IList]$AllRows,
    ...
    for ($r = 1; $r -lt $AllRows.Count; $r++) {
        ...
        $op  = if ($opIdx -ge 0 -and $opIdx -lt $row.Count) { $row[$opIdx] } else { '' }
        $met = if ($metIdx -ge 0 -and $metIdx -lt $row.Count) { $row[$metIdx] } else { '' }
        $match = switch ($Filter.ToUpper()) {
            'PPV'      { ($op -like 'PPV*') -or ([string]::IsNullOrWhiteSpace($op) -and $met -eq 'PPV-M SAMPLE SIZE') }
            'CLASS'    { ($op -like 'TEST*') -or ([string]::IsNullOrWhiteSpace($op) -and $met -in @('MPS Monitor','EQA Monitor','CS Monitor')) }
            'CLASSPPV' { ($op -like 'PPV*') -or ([string]::IsNullOrWhiteSpace($op) -and $met -eq 'PPV-M SAMPLE SIZE') -or ($op -like 'TEST*') -or ([string]::IsNullOrWhiteSpace($op) -and $met -in @('MPS Monitor','EQA Monitor','CS Monitor')) }
            'BI'       { $op -like 'BURNIN*' }
            'YIELD'    { [string]::IsNullOrWhiteSpace($op) -and $met -in @('U/D','R/D','FINISH YIELD') }
            default    { $op -like "$Filter*" }
        }
