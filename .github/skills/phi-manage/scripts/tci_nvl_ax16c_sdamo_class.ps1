...
Write-Host "=== $product - SDA Monthly + Class ===" -ForegroundColor Cyan
...
# --- CSV + Excel ---
$csvFile  = Join-Path $env:TEMP "${safeName}_SDAMO_Class.csv"
$xlsxFile = Join-Path $env:TEMP "${safeName}_SDAMO_Class.xlsx"

Tci-WriteCsv -Rows $filtered -Path $csvFile
$result = Tci-ExportXlsx -CsvPath $csvFile -XlsxPath $xlsxFile -SheetName 'SDA Monthly Class' -FreezeColumns 4 -Validate -MinRows 1
...
$subj = "PHI of $product - SDA Monthly Class"
