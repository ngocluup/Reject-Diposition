...
# --- CSV + Excel ---
$csvFile  = Join-Path $env:TEMP "${safeName}_SDA_PPV.csv"
$xlsxFile = Join-Path $env:TEMP "${safeName}_SDA_PPV.xlsx"

Tci-WriteCsv -Rows $filtered -Path $csvFile
$result = Tci-ExportXlsx -CsvPath $csvFile -XlsxPath $xlsxFile -SheetName 'SDA Weekly PPV' -FreezeColumns 4 -Validate -MinRows 1
...
# --- Email ---
$sections = @( @{ SheetTag = 'SDA Weekly PPV'; RowCount = $filtered.Count - 1 } )
$body = Build-PhiCard -Product $product -Group $grp -SubGroup $sub -Sections $sections -Filter $filter
$subj = "PHI of $product - SDA Weekly PPV"
